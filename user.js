...
MongoClient.connect(url, function(err, client) {
    ...
    var db = client.db("FoodDelivery");

    // Check if the database exists
    db.admin().listDatabases((err, dbs) => {
        if (err) throw err;

        const databaseExists = dbs.databases.some(db => db.name === "FoodDelivery");
        if (databaseExists) {
...
app.post("/userssearch", async function(req, res) {
    try {
        ...
        const filePath = path.resolve(__dirname, 'shops.html');
      ...
      app.post("/usersinsert", async function(req, res1) {
        try {
            ...
            const Password1 = req.body.cpassword;
            console.log("Input: " + Username + ", " + Userid + ", " + Email+","+Password1);
            ...
            const document = { Name: Username, id: Userid, Mail: Email, password: Password1 };
